...
class ComplexBatchNorm2d(_ComplexBatchNorm):
    ...
    def forward(self, input_r, input_i):
        ...
        if self.training:
            ...
            with torch.no_grad():
                self.running_mean = exponential_average_factor * mean\
                    + (1 - exponential_average_factor) * self.running_mean.cuda()
            ...
            with torch.no_grad():
                self.running_covar[:,0] = exponential_average_factor * Crr * n / (n - 1)\
                    + (1 - exponential_average_factor) * self.running_covar[:,0].cuda()

                self.running_covar[:,1] = exponential_average_factor * Cii * n / (n - 1)\
                    + (1 - exponential_average_factor) * self.running_covar[:,1].cuda()

                self.running_covar[:,2] = exponential_average_factor * Cri * n / (n - 1)\
                    + (1 - exponential_average_factor) * self.running_covar[:,2].cuda()
        ...
        if self.affine:
            input_r, input_i = self.weight[None,:,0,None,None]*input_r.cpu()+self.weight[None,:,2,None,None]*input_i.cpu()+\
                               self.bias[None,:,0,None,None], \
                               self.weight[None,:,2,None,None]*input_r.cpu()+self.weight[None,:,1,None,None]*input_i.cpu()+\
