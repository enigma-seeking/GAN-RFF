...
if __name__ == '__main__': 
    real_pre_data = np.load("../RFF_Identify/rDATA_rand_point.npy")  # 10000个sample
    ...
    for i in range(100):  # 将40万个点分为4000组100个点
        ...
        r = data_process(r, 1 / 30, 1 / 40, IQreal_Tphase, real_Talpha, IQfake_Rphase, fake_Ralpha,SNR)  # 合法发射机 --非法接收机   为了协助攻击
        rDATA_GAN = np.hstack((rDATA_GAN, r))

    rDATA_input = data_union(rDATA_GAN)
    print(rDATA_input.shape)
    rDATA_input = rDATA_input.reshape(10000, 900)
    real_lable = np. ones((10000, 1))
    rDATA_GANinput = np.hstack([real_lable, rDATA_input])
    GAN_train_data = np.concatenate((rDATA_GANinput[0:8000, :], rDATA_GANinput[10000:18000, :]), axis=0)
    GAN_test_data = np.concatenate((rDATA_GANinput[8000:10000, :], rDATA_GANinput[18000:20000, :]), axis=0)
    print(rDATA_GANinput.shape)
    np.save("GAN_train_data", GAN_train_data)
    np.save("GAN_test_data", GAN_test_data)
    np.save("rDATA_GANinput", rDATA_GANinput)   #还是复数，未经过拆分。这个数据集是到达真实数据到达攻击协作者的。
